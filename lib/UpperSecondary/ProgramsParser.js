...
function parseOrientations(xmlGroup, subjects, courses, degrees, orientationText, xmlProgram) {
  if(!xmlGroup) return [];
  let context = xmlProgram.name;
  ...
  for(let xmlOrientation of xmlGroup.programOrientation) {
    ...
    let text = "";
    for(let str of orientationText) {
      if(str.toLowerCase().indexOf(xmlOrientation.name.toLowerCase()) >= 0) {
        text = str;
        break;
      }
    }
  ...
  return Promise.all(xmlParsePromises)
    .then((xmlFiles) => {
      ...
      for(let xml of xmlFiles) {
        ...
        let orientationText = createListFromHTMLPTags(xmlProgram.purpose.orientation.content, xmlProgram);
        ...
        let program = {
          ...
          orientations: parseOrientations(xmlProgram.programOrientations, subjects, courses, degrees, orientationText, xmlProgram),
