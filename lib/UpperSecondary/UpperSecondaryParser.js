...
module.exports.parseXML = function parseXML(directory) {
  return readGlobFiles(directory, "*/subjectsAndCourses/*.xml")
    .then(files => {
      return ListParser(files).then(subjectsAndCourses => {
        return readGlobFiles(directory, "*/subject/*.xml")
          .then(subjectFiles => {
            return SubjectsParser(subjectFiles, subjectsAndCourses.subjects, subjectsAndCourses.courses);
          });
      });
    });
}
