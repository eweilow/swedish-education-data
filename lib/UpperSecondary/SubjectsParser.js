...
function parseDescription(description, context) {
  ...
  for(let element of tags.contents().toArray()) {
    text.push(element.data);
  }
  return text.join("\n");
}
function parsePurpose(purpose, code, context) {
  ...
  if($("h4").length !== 1 || $("h3").length !== 1 || $("l").length !== 2) {
    var beautify_html = require('js-beautify').html;
    let html = beautify_html(purpose);
      require("fs").writeFileSync("./tmp/" + code + ".html", html);
  }

  //Read until the first <h4> or the first <l>
  /*assert.equal($("ul,ol,dl").length, 0, context);
  assert.equal($("l").length, 2, context);
  let start = $("p:not(l~p)");
  let list = $("l").first();
  let courses = $("l").last();
*/
  //assert.equal(list.length, 1, context);
  //assert.equal(courses.length, 1, context);
}
  ...
  return Promise.all(xmlParsePromises)
    .then((xmlFiles) => {
      for(let xml of xmlFiles) {
        ...
        const subject = subjects.get(xmlSubject.code);
