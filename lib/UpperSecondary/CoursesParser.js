...
function parseCentralContent(centralContent, xmlCourse) {
  centralContent = centralContent.replace(/<ul>/g, "<l>").replace(/<\/ul>/g, "</l>").replace(/<i>/g, "").replace(/<\/i>/g, "")
  ...
  let notValidelements = $(":not(p):not(h4):not(l):not(li):not(strong):not(italic):not(br):not(b)");
...
function parseKnowledgeRequirements(knowledgerequirements, xmlCourse) {
  ...
  for(let gradeStep of knowledgerequirements) {
    ...
    assert.equal(tags_p.length + tags_h4.length, children.length, context + children.toArray().map(el => el.tagName)); //Make sure there's only one h4 and the rest p tags in the root scope.
